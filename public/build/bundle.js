    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t1 = space();
    			input = element("input");
    			add_location(label, file, 201, 2, 5779);
    			attr_dev(input, "id", "slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "1");
    			attr_dev(input, "max", "1440");
    			add_location(input, file, 202, 2, 5812);
    			attr_dev(div, "class", "overlay");
    			add_location(div, file, 200, 4, 5754);
    			add_location(main, file, 199, 0, 5742);
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	function update_station_markers() {
    		station_markers.transition().duration(1000).attr("r", function (d) {
    			...
    			let colorScale = d3.scaleLinear().domain([0, 1]).range(["blue", "red"]); // Ratio varies from 0 to 1
    			// Colors for low and high ratios
